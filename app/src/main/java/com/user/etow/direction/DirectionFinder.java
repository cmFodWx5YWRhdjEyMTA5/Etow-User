...
public class DirectionFinder {
    ...
    private void parseJSon(String data) throws JSONException {
        ...
        for (int i = 0; i < jsonRoutes.length(); i++) {
            ...
            JSONArray jsonArraySteps = jsonLeg.getJSONArray("steps");
            route.steps = new ArrayList<>();
            for (int j = 0; j < jsonArraySteps.length() - 1; j++) {
                JSONObject jsonObjectStep = jsonArraySteps.getJSONObject(j);
                JSONObject jsonObjectDistanceStep = jsonObjectStep.getJSONObject("distance");
                Distance distanceStep = new Distance(jsonObjectDistanceStep.getString("text"),
                        jsonObjectDistanceStep.getInt("value"));
                String strManeuver = "";
                if (jsonObjectStep.has("maneuver")) {
                    strManeuver = jsonObjectStep.getString("maneuver");
                }
                Step step = new Step(distanceStep, strManeuver);
